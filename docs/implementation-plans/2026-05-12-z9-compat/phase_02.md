...
- `successCodes: false` (or the Z9 equivalent) is required so a 400 resolves rather than throws. If `Zotero.HTTP.request`'s option name differs in Z9, adjust per `zotero-types@4.1.2`; the *intent* is "do not treat 4xx as an exception."
...
npm run test
```
Expected (network up, HTTP server enabled): the spec output shows the preflight, four contract `it`s, two HTTP `it`s, and the smoke `it` as `✔`, and the run reports **`10 passing`** with **no `pending` line (or `0 pending`)** and **`0 failing`**, exit 0. If Task 0 re-scoped AC9: **`8 passing`**. A `pending`/skipped test, or a non-zero exit, fails this gate — a passing *count* alone is not sufficient.
