...
| AC | Type | Command / file | Expected outcome | Phase |
|---|---|---|---|---|
| AC-Aggregate | integration | `npm run test` | exits `0` **and** reports `0 pending` **and** `0 failing`. Target `10 passing` (network up, HTTP server enabled) or `8 passing` (AC9 re-scoped). A passing *count* alone does not gate — a `pending`/skipped test fails the gate | Phase 2 (Task 7) |
