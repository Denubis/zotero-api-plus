...
**To shatter it:** Build the `.xpi` (`npm run build`; the artefact lands under `.scaffold/build/`), open Zotero 9, navigate to *Tools → Plugins*, install the `.xpi` via the gear menu's "Install Add-on From File…" option, and observe whether Zotero accepts the install.
...
1. `curl http://127.0.0.1:23119/api/plus` — body exactly `Zotero Local API Plus is running.`, `Content-Type: text/plain` (verifies AC7.1 end-to-end; complements the Phase 2 in-process and HTTP-dispatch tests, which run against the *dev* profile).
